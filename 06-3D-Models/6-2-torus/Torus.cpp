...
void Torus::init() {
	...
	// 1 Calculate the 2D ring
	for (int i = 0; i < prec + 1; i++) {
		float amt = toRadians(i * 360.0f / prec);

		// 1.1 Create a 2D ring
		// 1.1.1 Rotation matrix along z-axis
		glm::mat4 rMat = glm::rotate(glm::mat4(1.0f), amt, glm::vec3(0.0f, 0.0f, 1.0f));

		// 1.1.2 Creating a circle by rotating along z-axis, the length of radius is outer
		glm::vec3 initPos(rMat * glm::vec4(outer, 0.0f, 0.0f, 1.0f));

		// 1.2 Calculate coordinates
		// 1.2.1 Move the circle along the x-axis with the length of inner and calculate the verices coordinates
		vertices[i] = glm::vec3(initPos + glm::vec3(inner, 0.0f, 0.0f));

		// 1.2.2 Calculate texture coordinates
		texCoords[i] = glm::vec2(0.0f, ((float)i / (float)prec));


		// 1.2.3 Calculate tangent coordinates
		...
		// 1.2.4 Calculate normal coordinates
		normals[i] = glm::cross(tTangents[i], sTangents[i]);
	}

	// 2 Calculate the 3D torus
	for (int ring = 1; ring < prec + 1; ring++)
	{
		for (int i = 0; i < prec + 1; i++)
		{
			// 2.1 Calculate vertice coordinates
			...
			// 2.2 Calculate texture coordinates
			...
			// 2.2 Calculate tangent coordinates
